...
public class XRSceneTransitionManager : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        {
            Debug.LogWarning("Detected rouge XRSceneTransitionManager. Deleating it.");
    ...
    static public void PlaceXRRig(Scene xrScene, Scene newScene)
    {
        ...
        GameObject xrRigOrgin = newSceneObjects.First((obj) => { return obj.CompareTag("XRRigOrgin"); });

        if (xrRig && xrRigOrgin)
        {
            xrRig.transform.position = xrRigOrgin.transform.position;
            xrRig.transform.rotation = xrRigOrgin.transform.rotation;
        }
